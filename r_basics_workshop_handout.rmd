...
The order of the factor levels affects the plot.  In the last two plots you saw that `Cold` came before `Hot` in the legend.  That's because `qplot()` uses the default factor level order and `C` comes before `H` in the alphabet.  

We can change the order by manually changing `tempgroup` to a factor and setting the level order like we learned to do earlier.  Here let's make `Hot` come before `Cold`.
