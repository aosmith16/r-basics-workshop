...
When you open an R script you'll see the fourth pane show up.  Open up the R script `r_basics_workshop.R` now to see this.  You can navigate to it as you normally would on your computer and then open it (selecting RStudio if you need to choose a program) or navigate through the RStudio `Files` pane (the lower right pane in the picture above).  (To use the latter option you may need the `...` button on the far right, depending on what your default directory is.)
...
My impression is that many people start out using the default settings in RStudio.  That's absolutely fine to do.  However, there are many options you can set to personalize your RStudio version.  For example, I like to have my `Console` and `Source` panes side by side rather than on top of each other.  You can change this (and much more) in the `Global Options...` in the `Tools` menu.

```{r global, echo = FALSE}
knitr::include_graphics(here::here("images", "global_options.png") )
```

The fourth option down is where you can control the panes layout.  You should explore more of the global options, such as `Appearance`, and see what you like.
...
One important thing to talk about as you are first learning R is how to get R help.  I think learning this will help you more than anything else I can teach you.  
...
For example, if we wanted to take an average of some numbers with the `mean()` function, we would type `?mean` at the `>` in the R Console and pressing Enter.  The help page for the function opens in the `Help` pane on the lower right.  
...
I often spend time in the `Usage` section, since this is where defaults to argument are given.  The arguments are the labels shown within the function.  You might note here that the `na.rm` argument in `mean()` defaults to `FALSE`, which we'll talk more about later.

```{r help3, echo = FALSE}
knitr::include_graphics(here::here("images", "help_section2.png") )
```

The arguments the function takes and a description of those arguments is given in the `Arguments` section.  This is a section I often spend time in, figuring out what arguments do and the options for each argument.
...
After looking at `Usage` and `Arguments` I often jump right down to the `Examples` section to see what the code looks like.  The `Examples` section for `mean()` is pretty sparse, but you'll find that these are quite extensive for some functions.  

```{r help5, echo = FALSE}
knitr::include_graphics(here::here("images", "help_section4.png") )
```

There can be a variety of different information after `Arguments` and before `Examples`, such as a description of what is returned, mathematical notation, or references in support of what the function does.  This can be extremely valuable information, but I often don't read it until I run into trouble or need more information.  
...
If you've already opened the R script you'll be using in the RStudio `Source` pane, as you have today, you can use the overall drop-down menus: 
```
...
What this all means is that we will need to tell R that our dataset contains column names via the `header` argument.  This is commonly how we would store files, and it means that the very first row of our dataset has all the variable names in it.  
...
Uh-oh, I see a problem right away.  The `str()` function tells us what kind of variable each column in the dataset contains.  `DryWt` should be numeric, but R read it in as a factor (it will be read as a *character* by default starting in R 4.0.0).  A factor in R is a type of *classification* or *categorical* variable.
...
If you click on the `temperature` object in your RStudio `Environment` pane, you can see the dataset in your `Source` pane.  You cannot edit from here, but this is another way that you can get a sense of what the dataset looks like.  You can also do some basic filtering via this RStudio method.
...
This means we don't need to specify this in our code because our dataset does contain variable names in the first row.
...
You need to know the package name that you want to install and type it in the **Packages** field in the pop-up window.  
...
Unlike package installation, you do need to load add-on packages each time you use a new R session.  Here's a nice picture by [Dianne Cook](https://twitter.com/visnut/status/1248166446867181568) that explains `install.packages()` vs `library()` well.

```{r install-lib, echo = FALSE}
knitr::include_graphics(here::here("images", "install.v.lib.png") )
```
...
The main difference in loading Excel documents with `read_excel()` compared to what we've done so far is that we'll need to tell R which worksheet we want to to read in.  We can do this by giving either the index (`1` in this case, as it's the very first sheet) or name (`Sheet1`) of the sheet via the `sheet` argument.  It seems easiest in this simple case, where there's only one worksheet, to use the index.  We'll name the new dataset `respfall`.
...
With that in mind, let's change the `Date` variable to a date in both datasets with the function `as.Date()`.  While we will not go into this in great detail, I do think it is good for you to see this.  I've seen many people struggle with dates in R when they are first getting started.
...
Notice that I'm replacing the variable in `respspring` with the new variable by assigning it to the same name.  If I didn't name this variable as I changed it from a factor to a date, these changes would *not* take place.  Instead the output would show in the `Console` with the original dataset would be unchanged.
...
As I mentioned earlier, we want to combine these two datasets by putting one on top of the other.  Let's add a column to each of them to represent `season` prior to combining them.  It turns out this isn't too hard.  We define a new variable name in the dataset and assign whatever values we want to that new variable using dollar sign notation.
...
The function `rbind` stacks all the rows in the datasets based on matching names, which you would see if you delved deeply into the `Details` section of the help file.  This is a time where reading the information between the `Arguments` and the `Examples` was useful.
...
Check your Environment pane, though.  The `temperature` dataset has one less row than `respall`.  Which `Sample` is missing from `temperature`?  Let's check.
...
Package **dplyr** is already installed on the computers in the lab classroom but you may need to install it if you are working on this on your own computer.  You saw how to install packages [earlier in this workshop](#installing-an-add-on-package).  Once it is installed, load the package into R via the `library()` function
...
If we scroll down in the documentation we can see the an anti join will:

> return all rows from x where there are not matching values in y, keeping just columns from x.

In other words, the `anti_join()` returns rows from the first dataset listed that are **NOT** in the second dataset.  We want to see which value of `Sample` is in the `respall` dataset that is NOT in the `temperature` dataset.  This means `respall` will be our *x* dataset in `anti_join()`.  
...
Now we know the `temperature` dataset is missing sample 21.

### Using `merge()` to join two datasets

We can join all the temperature and respiration information into a single dataset using the `merge()` function, which is part of base R.  Another option would be the other *dplyr* join functions, which will be faster on large datasets.

```{r merge, eval = FALSE}
?merge
```

By default, the `merge()` function merges on all columns that have the same name in two datasets.  We'll name the merged dataset `resptemp`, and check out the first six lines and structure to see what it looks like.
...
You can also choose which column names to merge on, which I often do for code clarity, with the `by` argument.

```{r merge3, eval = FALSE}
merge(respall, temperature, by = "Sample")
```

The above works if the names in the two datasets are the same.  What if they are different?  Let's test by making a second temperature dataset called `temp2`, and change the name of `Sample` to `Samplenum`.
...
Now we'll need to define the variable to merge on in the first dataset (the `x` dataset) and in the second dataset (the `y` dataset) by using `by.x` and `by.y`.

```{r merge5, eval = FALSE}
merge(respall, temp2, by.x = "Sample", by.y = "Samplenum") 
```

Did you notice there are only 59 rows in `resptemp`?  This is because `merge()` drops any unmatched rows by default.  Because we are missing sample 21 in the temperature dataset, R dropped this row from the merged dataset.  To change this, we can set the `all` argument to `TRUE` to leave all rows in the output regardless of if they have a match in both datasets.  Missing values are filled in with `NA`.

```{r merge6}
head(resptemp)  # You can see that sample 21 is missing
resptemp = merge(respall, temperature, all = TRUE)
...
We won't be going into great detail on this today, but I will show you how to do some basic data exploration tasks.  If you want to learn more about data manipulation in R you can check out, e.g., my data manipulation workshop materials [here](https://ariel.rbind.io/workshop/datamanip/).

### Subset a dataset with `subset()`

We can make summaries separate by group once we learn how to use the `subset()` function.  See `?subset` to look at the documentation.

Here is an example of its use, where we print a subset of the dataset into the R Console only when `tempgroup` is `Cold`.  We use the `==` to *test for equality*.  R will test every row to see if the value of `tempgroup` is `Cold`.  If it is, the row will be kept; if not, it will be discarded.
